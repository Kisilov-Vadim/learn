...
- **Per-subject rules** — apply only to the selected subject. They arrive in the
  `get_subject_context` response as a `rules` array (each `{ id, text, active }`). Honor them
  whenever you are working that subject.

Only active rules are delivered; treat every delivered rule as a standing user instruction
that overrides your default behavior where they conflict (user instructions win).
...
- `add` — `p_action:"add"`, `p_text:"<rule>"`, and optional `p_subject_id` (omit for a
  global rule).
- `update` — `p_action:"update"`, `p_rule_id:"<id>"`, and `p_text` and/or `p_active`
  (set `p_active:false` to disable a rule without deleting it).
- `delete` — `p_action:"delete"`, `p_rule_id:"<id>"`.
...
global if it applies everywhere, subject-scoped if it's specific to the current subject.
```
